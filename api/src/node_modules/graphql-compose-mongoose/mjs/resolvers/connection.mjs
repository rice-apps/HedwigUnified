...
export default function connection(model, // === MongooseModel
tc, opts) {
  try {
  ...
  const _ref = opts || {},
    ...
    edgeTypeName
        ...
        sortOptions = _objectWithoutProperties(_ref, ["connectionResolverName", "findResolverName", "countResolverName", "edgeFields", "edgeTypeName"]);

  return prepareConnectionResolver(tc, {
    connectionResolverName,
    findResolverName,
    countResolverName,
    sort: _objectSpread(_objectSpread({}, sortConfigs), sortOptions),
    edgeFields,
    edgeTypeName
  });
}
export function prepareCursorQuery(rawQuery, cursorData, indexKeys, indexData, nextOper, prevOper) {
