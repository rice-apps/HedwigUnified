...
describe('integration tests', () => {
  ...
  describe('check subdocuments', () => {
    ...
    it('should return subdocument if it is non-empty', async () => {
      const UserTC = composeWithMongoose(UserModel); // UserTC.get('$findById.subDoc').extendField('field2', {
      //   resolve: (source) => {
  ...
  describe('projection', () => {
    ...
    it('should request all fields to rawData field', async () => {
      const res = await graphql(schema, '{ user(_id: "100000000000000000000000") { rawData } }');
