...
function ProductDetail () {
  ...
  ] = useMutation(FIND_OR_CREATE_CART, { variables: { vendorID: vendorID } })

  const {
    data: productData,
    loading: productLoading,
    error: productError
  } = useQuery(GET_PRODUCT_DETAIL, { variables: { _id: product } })
  ...
  const optionSets = [
    { title: 'Size', multi: false, variants: ['M', 'L'] },
    { title: 'Toppings', multi: true, variants: ['Boba', 'Oreo', 'Lychee'] }
  ]
  return (
    <div className='heroImage'>
      <div className='productInfo'>
        ...
        {optionSets.map(optionSet => {
          return <OptionSet optionSet={optionSet} />
        })}
        <h2>Amount</h2>
        <h1>{count}</h1>
        <div>
        ...
        <button onClick={() => history.push(`/user/vendors/${slug}/cart`)}>
          Go to Cart
        </button>
      </div>
    </div>
