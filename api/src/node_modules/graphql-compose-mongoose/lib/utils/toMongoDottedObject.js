...
const primitives = [_mongoose.Types.ObjectId, Date, String, Number, Boolean, _mongoose.Types.Decimal128];
...
function _toMongoDottedObject(obj, target = {}, path = [], filter = false) {
  if (!(0, _graphqlCompose.isObject)(obj) && !Array.isArray(obj)) return obj;
  ...
  objKeys.forEach(key => {
    if (key.startsWith('$')) {
      ...
      if (operatorsWithExpression.includes(key)) {
        val = Array.isArray(obj[key]) ? obj[key].map(v => _toMongoDottedObject(v, {}, [], filter)) : _toMongoDottedObject(obj[key], {}, [], filter);
      } else {
        val = obj[key];
      }
      ...
      if (path.length === 0) {
        ...
        target[path.join('.')] = _objectSpread(_objectSpread({}, target[path.join('.')]), {}, {
          [key]: val
        });
      }
...
function toMongoFilterDottedObject(obj, target = {}, path = []) {
