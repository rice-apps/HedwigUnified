...
mutation($name: String!, $phone: String!, $netid: String!) {
  userUpdateOne (record: {name: $name, phone: $phone}, filter: {netid: $netid}) {
    record {
      name 
      phone
    }
...
function ContactForm() {
  ...
  const userName = sStorage.getItem('first name') + ' ' + sStorage.getItem('last name')
  ...
  return (
    <div>
      ...
      <div className="tel-container"> 
        <TextField type="tel" label="phone number:" id="tel" margin="dense" onChange={e => 
          setPhone(e.target.value)}></TextField>
      </div>
      <div variant="outlined" className="confirm-btn" onClick={() => {
