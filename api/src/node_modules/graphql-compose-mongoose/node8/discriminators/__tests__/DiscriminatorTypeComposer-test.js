...
var _movieModel = require("../__mocks__/movieModel");
...
describe('DiscriminatorTypeComposer', () => {
  ...
  describe('DInterface', () => {
    afterAll(() => _graphqlCompose.schemaComposer.clear());
    const baseDTC = (0, _composeWithMongooseDiscriminators.composeWithMongooseDiscriminators)(CharacterModel);
    it('should have same field names as baseModel used to create it', () => {
      expect(baseDTC.getFieldNames()).toEqual(expect.arrayContaining(Object.keys(baseDTC.getDInterface().getFields())));
    });
    it('should be accessed with getDInterface', () => {
      expect(baseDTC.getDInterface()).toBeInstanceOf(_graphqlCompose.InterfaceTypeComposer);
    });
  ...
  describe('class methods', () => {
    ...
    describe('hasChildTC(DName)', () => {
      beforeAll(() => {
        ...
        characterDTC = (0, _composeWithMongooseDiscriminators.composeWithMongooseDiscriminators)(CharacterModel);
    ...
    describe('setFields(fields)', () => {
      ...
      it('should sets fields to DInterface', () => {
        expect(Object.keys(characterDTC.getDInterface().getFields())).toEqual(Object.keys(fieldsToSet));
      });
    ...
    describe('setField(fieldName, fieldConfig)', () => {
      ...
      it('should set field on baseTC', () => {
        expect(characterDTC.getFieldType(fieldName)).toEqual(_graphqlCompose.graphql.GraphQLString);
      });
      it('should set field on DInterface', () => {
        expect(characterDTC.getDInterface().getFieldType(fieldName)).toEqual(_graphqlCompose.graphql.GraphQLString);
      });
      it('should set field on childTC', () => {
        expect(droidTC.getFieldType(fieldName)).toEqual(_graphqlCompose.graphql.GraphQLString);
        expect(personTC.getFieldType(fieldName)).toEqual(_graphqlCompose.graphql.GraphQLString);
      });
    });
    describe('addFields(newFields)', () => {
      ...
      it('should add fields to DInterface', () => {
        expect(Object.keys(characterDTC.getDInterface().getFields())).toEqual(expect.arrayContaining(Object.keys(newFields)));
      });
      it('should have exactly plus two fields added to childTC fields', () => {
        expect(droidTC.getFieldNames().sort()).toEqual([...characterDTC.getFieldNames(), ...droidSpecificFields].sort());
        expect(personTC.getFieldNames().sort()).toEqual([...characterDTC.getFieldNames(), ...personSpecificFields].sort());
      });
    });
    describe('addNestedFields(newFields)', () => {
      ...
      it('should add field to baseTC', () => {
        expect(characterDTC.getFieldOTC('field1').getFieldType('nested')).toEqual(_graphqlCompose.graphql.GraphQLString);
      });
      it('should add field to DInterface', () => {
        expect(characterDTC.getDInterface().getFieldOTC('field1').getFieldType('nested')).toEqual(_graphqlCompose.graphql.GraphQLString);
      });
      it('should have exactly plus two fields added to childTC fields', () => {
        expect(droidTC.getFieldOTC('field1').getFieldType('nested')).toEqual(_graphqlCompose.graphql.GraphQLString);
        expect(personTC.getFieldOTC('field2').getFieldType('nested')).toEqual(_graphqlCompose.graphql.GraphQLString);
      });
    ...
    describe('extendFields(fieldName, extensionField)', () => {
      ...
      it('should extend field on baseTC', () => {
        expect(characterDTC.getFieldType(fieldName).toString()).toEqual(_graphqlCompose.graphql.GraphQLString.name);
      ...
      it('should extend field type on DInterface', () => {
        ...
        expect(characterDTC.getDInterface().getFieldType(fieldName).toString()).toEqual(fieldExtension.type);
      });
    ...
    describe('makeFieldNonNull(fieldName), makeFieldNullable(fieldName), deprecateFields(fieldName)', () => {
      ...
      beforeAll(() => {
        ...
        characterDTC.deprecateFields({
          [fieldDeprecated]: deprecateMessage
        });
      });
      it('should effect fields on baseTC', () => {
        ...
        expect(characterDTC.getFieldConfig(fieldDeprecated).deprecationReason).toBe(deprecateMessage);
      });
    ...
    describe('addRelation(fieldName, relationOpts)', () => {
      ...
      const relationResolver = (0, _composeWithMongoose.composeWithMongoose)(_movieModel.MovieModel).getResolver('findMany');
      beforeAll(() => {
  ...
  describe('discriminator()', () => {
    ...
    it('should return an instance of ObjectTypeComposer as childTC', () => {
      expect(characterDTC.discriminator(PersonModel)).toBeInstanceOf(_graphqlCompose.ObjectTypeComposer);
      expect(characterDTC.discriminator(DroidModel)).toBeInstanceOf(_graphqlCompose.ObjectTypeComposer);
    });
    ...
    it('should pass down baseTypeComposerResolverOptions', () => {
      const personTC = (0, _composeWithMongooseDiscriminators.composeWithMongooseDiscriminators)(CharacterModel, {
