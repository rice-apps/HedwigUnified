...
function prepareConnectionResolver(tc, opts) {
  if (!(tc instanceof _graphqlCompose.ObjectTypeComposer)) {
    throw new Error(`First arg for prepareConnectionResolver() should be instance of ObjectTypeComposer but recieved: ${(0, _graphqlCompose.inspect)(tc)}`);
  }
  ...
  const sortEnumType = (0, _sortInputType.prepareSortType)(tc, opts);
  const firstField = sortEnumType.getFieldNames()[0];
  const defaultValue = firstField && sortEnumType.getField(firstField).value;
  return tc.schemaComposer.createResolver({
    type: (0, _connectionType.prepareConnectionType)(tc, opts.connectionResolverName, opts.edgeTypeName, opts.edgeFields),
...
function prepareRawQuery(rp, sortConfig) {
  ...
  const beginCursorData = (0, _cursor.cursorToData)(rp.args.after);
  ...
  const endCursorData = (0, _cursor.cursorToData)(rp.args.before);
...
function prepareLimitSkipFallback(rp, limit, skip) {
  ...
  if (rp.args.before) {
    const tmp = (0, _cursor.cursorToData)(rp.args.before);
  ...
  if (rp.args.after) {
    const tmp = (0, _cursor.cursorToData)(rp.args.after);
