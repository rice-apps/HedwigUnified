...
class TypeMapper {
  ...
  convertOutputFieldConfig(composeFC, fieldName = '', typeName = '') {
    try {
      ...
      throw new Error(`Cannot convert to InputType the following value: ${(0, _misc.inspect)(composeFC)}`);
    } catch (e) {
  ...
  convertInterfaceTypeDefinition(typeDef) {
    if (this.schemaComposer.hasInstance(typeDef, _InterfaceTypeComposer.InterfaceTypeComposer)) {
      ...
      return new _ThunkComposer.ThunkComposer(() => this.convertInterfaceTypeDefinition(typeDef()));
    }
