...
import { GraphQLNonNull, GraphQLInputObjectType, getNullableType } from 'graphql-compose/lib/graphql';
...
describe('updateById() ->', () => {
  ...
  describe('Resolver.args', () => {
    ...
    it('should have `record._id` required arg', () => {
      ...
      if (argConfig.type && argConfig.type.ofType) {
        const _idFieldType = schemaComposer.createInputTC(argConfig.type.ofType).getFieldType('_id');
  ...
  describe('Resolver.resolve():Promise', () => {
    ...
    it('should change data via args.record in database', async () => {
      ...
      await expect(UserModel.findOne({
        _id: user1._id
    ...
    it('`beforeRecordMutate` may reject operation', async () => {
      ...
      await expect(result).rejects.toMatchSnapshot();
      const exist = await UserModel.collection.findOne({
        _id: user1._id
      });
